...
QString ConfigStrings::SHOWAUTOGENERATEDDOCS = QStringLiteral("showautogenerateddocs");
...
/*!
  Resets the Config instance and loads the default values. A subset
  of command line options are published as configuration variables.
 */
void Config::reset()
{
    clear();
    ...
    // Load default values
    load(Location(), u":/qdoc/config/defaults.qdocconf"_s);
...
/*!
  Load, parse, and process a QDoc configuration file \a fileName.
  This function is only called by the other load() function, but
  is recursive, i.e., it calls itself when it sees an \c include
  statement in the configuration file.

  The path of the loaded configuration file is pushed to a stack
  and set as the current working directory, unless \a fileName
  comes from the resource system. Configuration files from
  resources can only include() other files with an absolute path,
  or \c {:/path/to/qdocconf}.
 */
void Config::load(Location location, const QString &fileName)
{
    QFileInfo fileInfo(fileName);
    const bool isResource = fileName.startsWith(u':');
    if (!isResource)
...
#define PUT_CHAR()                                                                                 \
    ...
    QFile fin(isResource ? fileInfo.filePath() : fileInfo.fileName());
    if (!fin.open(QFile::ReadOnly | QFile::Text)) {
        if (!Config::installDir.isEmpty()) {
            qsizetype prefix = location.filePath().size() - location.fileName().size();
            fin.setFileName(Config::installDir + QLatin1Char('/')
...
bool Config::showAutoGeneratedDocs() const
{
    return m_configVars.value(CONFIG_SHOWAUTOGENERATEDDOCS).asBool();
}
...
InclusionPolicy Config::createInclusionPolicy() const
{
    ...
    policy.showAutoGeneratedDocs = showAutoGeneratedDocs();
