...
NodeContext Node::createContext() const
{
    ...
    context.hasAutoGeneratedDoc = hasDoc() && m_doc.isAutoGenerated();
...
/*!
  ...
  replaced silently. Auto-generated documentation (for explicitly
  defaulted or deleted functions) is also silently replaced.
 */
void Node::setDoc(const Doc &doc, bool replace)
{
    if (!m_doc.isEmpty() && !replace && !doc.isMarkedReimp() && !m_doc.isAutoGenerated()) {
...
{
    if (status() == Status::Internal || status() == Status::InternalAuto)
        return true;
    if (parent() && !parent()->isAbstract()) {
        const Status parentStatus = parent()->status();
        return parentStatus == Status::Internal || parentStatus == Status::InternalAuto;
    }
    return false;
}
