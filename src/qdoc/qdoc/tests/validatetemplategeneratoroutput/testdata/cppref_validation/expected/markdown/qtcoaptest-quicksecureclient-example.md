...
For a [CMake](qtcoaptest-quicksecureclient-example.md#cmake)-based build, add the following to the `CMakeLists.txt`:
```cpp
...
The CoAP test server will be reachable on ports _5683_ (non-secure) and _5684_ (secure). For instructions on retrieving the IP address see [Getting The IP Address](qtcoaptest-quicksecureclient-example.md#getting-the-ip-address).
